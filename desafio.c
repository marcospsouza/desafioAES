            ...
            for(key[13] = 33; key[13] <= 126; key[13]++){
                for(key[14] = 33; key[14] <= 126; key[14]++){
                    ...
                    for(int k = 33; k <= 126; k++){
                        ...
                        int alpha = 0;
                        for(int i = 0; i < 16; i++){
                            if ((decryptedtext[i] < 48 || decryptedtext[i] > 57) && (decryptedtext[i] < 65 || decryptedtext[i] > 90) && (decryptedtext[i] < 97 || decryptedtext[i] > 122) && decryptedtext[i] != 10 && decryptedtext[i] != 'á' && decryptedtext[i] != 'à' && decryptedtext[i] != 'ã' && decryptedtext[i] != 'â' && decryptedtext[i] != 'é' && decryptedtext[i] != 'ê' && decryptedtext[i] != 'ú' && decryptedtext[i] != 'í' && decryptedtext[i] != 'ç' && decryptedtext[i] != 'Á' && decryptedtext[i] != 'À' && decryptedtext[i] != 'Ã' && decryptedtext[i] != 'Â' && decryptedtext[i] != 'É' && decryptedtext[i] != 'Ê' && decryptedtext[i] != 'Ú' && decryptedtext[i] != 'Í' && decryptedtext[i] != 'Ç' && decryptedtext[i] != ',' && decryptedtext[i] != '.' && decryptedtext[i] != ' '){
                                alpha++;
                            }
